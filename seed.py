...
#         try:
#             users = [
#                 User(username='johndoe', email='johndoe@example.com', password='password123', role='user'),
#                 User(username='janedoe', email='janedoe@example.com', password='password123', role='user'),
#                 User(username='alice.smith', email='alice.smith@example.com', password='password123', role='user'),
#                 User(username='bob.jones', email='bob.jones@example.com', password='password123', role='user'),
#                 User(username='carol.johnson', email='carol.johnson@example.com', password='password123', role='user'),
#                 User(username='dave.williams', email='dave.williams@example.com', password='password123', role='user'),
#                 User(username='eve.davis', email='eve.davis@example.com', password='password123', role='user'),
#                 User(username='frank.miller', email='frank.miller@example.com', password='password123', role='user'),
#                 User(username='grace.martin', email='grace.martin@example.com', password='password123', role='user'),
#                 User(username='hank.lewis', email='hank.lewis@example.com', password='password123', role='user')
#             ]
        ...
        try:
            for user_data in users_data:
                user = User(
                   ...
                   password=user_data["password"]  # Ensure password is included
    ...
    with app.app_context():  # Ensure all database operations are within app context
        ...
        try:
            for event_data in events_data:
                ...
                db.session.add(event)
            db.session.commit()
            logging.info("Events seeded successfully.")
        except Exception as e:
            db.session.rollback()
            logging.error(f"Error seeding events: {e}")
...
if __name__ == "__main__":
    with app.app_context():  # Wrap the main block in app context
        delete_existing_data()
        seed_users()
        seed_events()
        seed_artworks()
